...
";
    ...
    let mut merge_distance: u32 = 500;
...
fn intermediate_regions(methfn: &str, outdir: &str, nadj: usize,
			region: &str, lb:f32, ub:f32) -> () {
    ...
    if "" == region {
        for contig in contigs {
            ...
            let msmaybe = utils::get_bedmethyl_slice(&mut tbxreader, &contig, 0, i64::MAX as u64);
        ...
        let msmaybe = utils::get_bedmethyl_slice(&mut tbxreader, &contig, start as u64, end as u64);
	let ms = match msmaybe {
		Ok(ms) => ms,
		Err(_) => return ()
	};
...
fn state_of_meth<W: std::io::Write>(handle: &mut BufWriter<W>,
				    ms: utils::MethSlice,
				    nadj: usize, lb:f32, ub:f32) {
